...
PS
{
	...
	CreateInputTexture2D( Trans, Srgb, 8, "None", "_trans", ",0/0", Default4( 0.00, 0.00, 0.00, 0.00 ) );
	...
	CreateTexture2DWithoutSampler( g_tTrans ) < Channel( RGBA, Box( Trans ), Srgb ); OutputFormat( DXT5 ); SrgbRead( True ); >;
	CreateTexture2DWithoutSampler( g_tNormal ) < Channel( RGBA, Box( Normal ), Srgb ); OutputFormat( DXT5 ); SrgbRead( True ); >;
	CreateTexture2DWithoutSampler( g_tRough ) < Channel( RGBA, Box( Rough ), Srgb ); OutputFormat( DXT5 ); SrgbRead( True ); >;
	...
	float4 MainPs( PixelInput i ) : SV_Target0
	{
		...
		float4 local35 = Tex2DS( g_tTrans, g_sSampler0, i.vTextureCoords.xy );
		float local36 = local35.r;
		float2 local37 = i.vTextureCoords.xy * float2( 1, 1 );
		float4 local38 = Tex2DS( g_tNormal, g_sSampler0, local37 );
		float4 local39 = Tex2DS( g_tRough, g_sSampler0, i.vTextureCoords.xy );
		float local40 = local39.r;
		float2 local41 = i.vTextureCoords.xy * float2( 1, 1 );
		float4 local42 = Tex2DS( g_tAO, g_sSampler0, local41 );

		m.Albedo = local31.xyz;
		m.Emission = local34.xyz;
		m.Opacity = local36;
		m.Normal = local38.xyz;
		m.Roughness = local40;
		m.AmbientOcclusion = local42.x;
